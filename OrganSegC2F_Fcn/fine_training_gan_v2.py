...
if __name__ == '__main__':
    ...
    restore_variables_gen = [v for v in tf.trainable_variables() if 'FCN_slice' in v.name]
    restore_variables_dis = [v for v in tf.trainable_variables() if 'FCN_dis' in v.name]

   
    model_file = checkpoint_path + train_plane + '/' + train_plane + '_' + str(current_fold) + '/' + 'fine-' + train_plane + '-' + str(current_fold)
    init_fn_gen = slim.assign_from_checkpoint_fn(model_path=model_file, var_list=restore_variables_gen, ignore_missing_vars=True)
    init_fn_dis = slim.assign_from_checkpoint_fn(model_path=model_file, var_list=restore_variables_dis, ignore_missing_vars=True)
