...
def FCN_model(images, labels, training, reuse=False, finetune=False):
    ...
    upsampled_logits_batch_1, _, fcn_8s_variables_mapping = FCN_8s(image_batch_tensor=images,
    ...
    up_loss, down_loss, up_DSC, down_DSC = DSC_loss(upsampled_logits_batch_1, pred0, labels)
...
if __name__ == '__main__':
    ...
    model_path = '/media/jionie/Disk1/checkpoints/coarse/fcn_vgg/X/X_0_22/fine-X-22'
    ...
    with tf.Session() as sess:
        ...
        init_fn(sess)
        num_epoch = round(1.5*max_iterations/len(data.active_index)) + 1

        for epochs in range(int(num_epoch)):
            data.shuffle_data()
            ...
            if is_save :
                ...
                if not os.path.exists(checkpoint_path + train_plane + '_' + str(current_fold) + '_' + str(epochs) + '_v2/'):     
                    os.makedirs(checkpoint_path + train_plane + '_' + str(current_fold) + '_' + str(epochs) + '_v2/')

                saver.save(sess, checkpoint_path + train_plane + '_' + str(current_fold) + '_' + str(epochs) + '_v2/' +'fine-' \
