...
if __name__ == '__main__':
    ...
    start_epoch = 12
    #start_iteration = 20393

    model_file = checkpoint_path + train_plane + '/' + train_plane + '_' \
        + str(current_fold) + '_' + str(start_epoch) + '/' + 'fine-' + train_plane + '-' + str(start_epoch)
    ...
    with tf.Session() as sess:
        ...
        train_writer = tf.summary.FileWriter(summary_save_dir1 + '_continue', sess.graph)
        eval_writer = tf.summary.FileWriter(summary_save_dir2 + '_continue', sess.graph)

        init_fn(sess) 

        print(len(data.active_index))

        num_epoch = round(5000/len(data.active_index)) + 1
        print(num_epoch)
            ...
            for iterations in range(len(data.active_index)):
                ...
                if (is_save and iterations % 500 == 0):
                    ...
                    if not os.path.exists(checkpoint_path + train_plane + '/' + train_plane + '_' \
                                          + str(current_fold) + '_' + str(start_epoch)  + '/' \
                    ...
                    saver.save(sess, checkpoint_path + train_plane + '/' + train_plane + '_' \
                                      + str(current_fold) + '_' + str(start_epoch)  + '/' \
                                      + str(epochs*len(data.active_index) + iterations) + '/'  +'fine-' \
